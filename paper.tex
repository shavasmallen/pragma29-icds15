...
\item The user selects the desired resources, the timeframe, virtual cluster image, and configuration details and submits them to the Cloud scheduler.  Once the reservation is verified and confirmed, the user will get an email confirmation.  Otherwise if there is a problem with the reservation such as someone else has taken the resources while the user was filling in the reservation fields, the user will get notified immediately of the failure once they hit the submit button or later via email.
...
\item Added virtual cluster statuses: "Starting", "Running", and "Stopping".  Each virtual cluster status is shown as a different color in  the calendar view.  "Starting" means it has been accepted by the scheduler and is being started up on the local cloud systems, "Running" means that the resources are available for the user to log into, and "Stopping" means the virtual cluster is being shutdown and is no longer available for log in.
...
We still need to address the scenario where two users might try to reserve the same resources at the same time as a race condition is possible.  Booked does not have are not any locking mechanisms in place currently that would handle two simultaneous user requests for the same resources gracefully and so this will need to be added in the future.    We also are not handling hardware failures at resource sites so when a  virtual cluster goes down; the failure would be recorded but we cannot accommodate relocation.  For fault tolerance of the cloud scheduler, we will deploy a backup instance somewhere else so that we can failover in the event of an outage.
