...
the same time, for example, one user has taken the resources while the second user was filling in the reservation fields, the second user will get 
a failure notification after hitting the submit button, and later via email.
...
The solution will need to be added in the future.  We also do not handle hardware failures at resource sites. When a virtual 
cluster goes down the failure will be recorded but we cannot accommodate a cluster relocation.  For fault tolerance of the cloud scheduler, we will
deploy a backup instance at a differnt site so that we can do a cloud scheduler server failover.
...
In the pilot implementation of the cloud scheduler  we made a lot of simplifying assumptions, and  our future work will address them.  
Currently, the virtual cluster startup latency from the reservation to its availability to the user has not been analyzed in detail; but we observed
it is on the order of tens of minutes.   Multiple factors influence this latency, such as the underlying virtualization manager and load on the
...
In the future when we use deployment on more sites, we can gather statistics on the cloud scheduler's usability and user experiences with virtual
...
cloud scheduler.  Next, we will rework the PCC software and integrate HTCondor so it is used in personal mode allowing us to keep a light footprint on
each of the PRAGMA sites.  Finally, we will investigate using the CloudFront option in pragma\_boot to manage application-specific  virtual cluster
